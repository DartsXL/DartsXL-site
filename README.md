...
**A dart counter and checkout routes analyzer for all major dartboard types — built on a novel real-time combinatorial algorithm.**
...
DartsXL Counter is a dart scoring counter with an integrated checkout routes analyzer, available as a web app and on Android.

### The Counter

A scoring counter for 501-rules darts with configurable settings for starting score and number of darts per visit. Designed to be picked up and used without any prior familiarity with the app. Records match states and produces checkout routes throughout.

### The Checkout Routes Analyzer

An advanced tool for evaluating checkout routes. The user can analyze checkout options from any recorded match state, branch off simulated "what if" scenarios from any point in a match, or enter a custom score to evaluate independently. The analyzer is built on the same algorithm as the counter but does substantially more work.
...
Every pre-existing dart counter application has solved this by brute force editorial work: someone manually enumerated the accepted "best" routes for every possible remaining score, entered them as hard-coded lookup tables, and manually ranked them. This is the universal prior art in the field.
